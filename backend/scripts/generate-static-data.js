...
function buildSitemap(topics, siteUrl) {
  ...
  // Every route but `/` is prerendered to <route>/index.html, and the host
  // 301s the no-slash URL to that directory. A sitemap must list the URL that
  // answers 200, so each <loc> carries the trailing slash (`/` already has
  // one from siteUrl + '/').
  const body = urls
    .map(
      (u) =>
...
function main() {
  ...
  if (sitemap) {
    ...
    // Disallow paths must carry the deploy's base path — the app is served
    // under /<repo>/, so a bare `/admin` guards a path nobody serves while
    // the real one stays crawlable. siteUrl already carries the base.
    const basePath = new URL(siteUrl).pathname.replace(/\/+$/, '');
    // The Sitemap directive must be an absolute URL.
    fs.writeFileSync(ROBOTS_FILE, `# Generated by backend/scripts/generate-static-data.js — do not hand-edit.\n# The Pages deploy runs the script again with its own SITE_URL, so the paths\n# below always match the base path that deploy is served under.\nUser-agent: *\nAllow: /\n\n# /admin is dev-only and not built for production; disallowed defensively.\nDisallow: ${basePath}/admin\n\nSitemap: ${siteUrl}/sitemap.xml\n`);
