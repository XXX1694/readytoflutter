...
export default function HomePage({ landing = null }: HomePageProps) {
  ...
  // The bare `/` had no canonical, so `/`, `/?stack=ios` and every other
  // query variant read as separate URLs to a crawler. The landings bring their
  // own (landings.ts); this is the fallback for the dashboard itself.
  useDocumentMeta({
