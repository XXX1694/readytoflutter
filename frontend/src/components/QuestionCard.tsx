...
import { Pill, difficultyTone } from '../ui/index';
...
const QuestionCard = forwardRef<HTMLElement, any>(function QuestionCard(
  { question, index, expanded: controlledExpanded, onToggleExpand, focused, topicSlug }: any,
  ref,
) {
  ...
  const [thisSpeakingToken, setThisSpeakingToken] = useState<any>(null);
  useEffect(() => {
...
function NotesEditor({ questionId, initialNotes, status, topicSlug }: any) {
  const { lang } = useLang();
  const t = useT(lang);
  const update = useUpdateProgress();
